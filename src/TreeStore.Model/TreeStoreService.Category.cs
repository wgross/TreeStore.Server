...
namespace TreeStore.Model
{
    ...
    public sealed partial class TreeStoreService : ITreeStoreService
    {
        ...
        /// <inheritdoc/>
        public Task<CategoryResult> MoveCategoryToAsync(Guid sourceCategoryId, Guid destinationCategoryId, CancellationToken cancellationToken)
        {
            var sourceCategory = this.model.Categories.FindById(sourceCategoryId);
            if (sourceCategory is null)
                throw new InvalidOperationException($"Category(id='{sourceCategoryId}') wasn't moved: Category(id='{sourceCategoryId}') doesn't exist");

            var destinationCategory = this.model.Categories.FindById(destinationCategoryId);
            if (destinationCategory is null)
                throw new InvalidOperationException($"Category(id='{sourceCategoryId}') wasn't moved: Category(id='{destinationCategoryId}') doesn't exist");

            destinationCategory.AddSubCategory(sourceCategory);

            return Task.FromResult(this.model.Categories.Upsert(sourceCategory).ToCategoryResult());
        }
